function getLineType(s) { // categorizes line as "blank", "chord" or "lyrics". 
	...
	for (var i=0; i<s.length; i++) 
		if (s[i] == " " || s[i] == "\t") {NumWhiteSpace++}
	
	var WhiteSpaceFraction = NumWhiteSpace/s.length;
	
	// check line to see if it contains chords
		var MajorChords = s.match(/\s[B-G]\s/);
		var MinorChords = s.match(/\s[B-G]m\s/);
		var SevenChords = s.match(/\s[A-G]7/);
		var SusChords   = s.match(/\s[B-G]sus/);
	 
	//return categorization based on number of white spaces
	if (WhiteSpaceFraction==1)
		return "blank"
	//else if (WhiteSpaceFraction > 0.5) {
	else if (MajorChords!=null || MinorChords!=null || SevenChords!=null || SusChords!=null) 
		return "chords"
	else
		return "lyrics"
}  
